...
// =======================================================
// 1. "ĐÔI TAI" - NGHE DỮ LIỆU TỪ MÂY ĐỂ HIỂN THỊ LCD
// =======================================================
    ...
    while (1) {
        ...
        if (!is_subscribed) {
            ...
            if (tb.RPC_Subscribe(first_callback, last_callback)) {
                ...
                // NẾU THẤT BẠI: Bắt buộc phải ngủ 3 giây rồi mới thử lại, cấm spam!
                if (xSemaphoreTake(xSerialMutex, portMAX_DELAY)) {
                    Serial.println(">> Dang ky RPC that bai. Thu lai sau 3 giay...");
                    xSemaphoreGive(xSerialMutex);
                }
                vTaskDelay(pdMS_TO_TICKS(3000));
                continue; // Bỏ qua đoạn dưới, quay lại đầu vòng lặp để thử đăng ký lại
            }
        }

        // =======================================================
        // 3. DUY TRÌ KẾT NỐI VÀ XỬ LÝ LỆNH
        // =======================================================
        tb.loop(); // Bắt buộc phải có để giữ nhịp tim MQTT và nhận RPC

        // =======================================================
        // 4. "CÁI MIỆNG" - KIỂM TRA CỜ TỪ KEYPAD VÀ BẮN LÊN CLOUD
        // =======================================================
        if (flag_toggle_mode) {
            tb.sendTelemetryData("cmd_toggle_mode", 1);
            if (xSemaphoreTake(xSerialMutex, portMAX_DELAY)) {
                Serial.println(">> [Node 2 -> Cloud] Yeu cau dao che do AUTO/MANUAL!");
                xSemaphoreGive(xSerialMutex);
            }
            flag_toggle_mode = false;
        }

        if (flag_toggle_heater) {
            tb.sendTelemetryData("cmd_toggle_heater", 1);
            if (xSemaphoreTake(xSerialMutex, portMAX_DELAY)) {
                Serial.println(">> [Node 2 -> Cloud] Yeu cau bat/tat den suoi!");
                xSemaphoreGive(xSerialMutex);
            }
            flag_toggle_heater = false;
        }

        if (flag_toggle_servo) {
            tb.sendTelemetryData("cmd_toggle_servo", 1);
            if (xSemaphoreTake(xSerialMutex, portMAX_DELAY)) {
                Serial.println(">> [Node 2 -> Cloud] Yeu cau dao trung!");
                xSemaphoreGive(xSerialMutex);
            }
            flag_toggle_servo = false;
        }
